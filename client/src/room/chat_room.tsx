...
export default function ChatRoom({
    ...
    const fetchMessageHistory = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8000/rooms/${roomName}/history`
        );
        if (response.ok) {
  ...
  const handleAIResponse = async (messageText: string) => {
    ...
    try {
      ...
      // Updated to include room_id for semantic context
      const response = await fetch("http://localhost:8000/chat", {
        ...
        body: JSON.stringify({
          user_id: username,
          message: query,
          room_id: roomName, // Include room_id for semantic context
        }),
      });
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    // Store regular messages in semantic memory too (not just AI interactions)
    // This helps build a richer context for future AI responses
    if (!inputMessage.includes("@AI")) {
      try {
        ...
        console.error("Error storing message in semantic memory:", error);
      }
    }
    ...
    <div className="flex flex-col h-screen bg-zinc-900 text-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 bg-zinc-800 p-4 overflow-y-auto">
          ...
          {/* Updated help text to explain semantic memory */}
          <div className="mt-4 p-3 bg-zinc-700 rounded-md">
            <h4 className="text-xs font-semibold text-zinc-300 mb-1">
              How to use AI:
            </h4>
            <p className="text-xs text-zinc-400">
              Type <span className="text-purple-400 font-mono">@AI</span>{" "}
              followed by your question to get AI assistance.
            </p>
            <p className="text-xs text-zinc-400 mt-1">
              Example:{" "}
              <span className="italic">@AI What's the weather like?</span>
            </p>
            <p className="text-xs text-zinc-400 mt-1">
              The AI remembers context from previous conversations and can
              answer questions about topics discussed earlier.
            </p>
          </div>
        </div>
          ...
          >
            {isLoadingHistory ? (
              ...
              <div className="space-y-4">
                ...
                {/* Messages with semantic indicator for AI responses */}
                {messages.map((msg) => (
                  <div
                    key={msg.id}
                    className={`flex ${
                  ...
                  >
                    ...
                    >
                      <div className="text-xs text-zinc-300 mb-1 flex justify-between items-center">
